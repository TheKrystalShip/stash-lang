...
/// <summary>Implements <c>stash service start</c>.</summary>
public static class StartCommand
{
    public static void Execute(string[] args)
    {
        if (args.Length == 0)
        {
            Console.Error.WriteLine("Usage: stash service start <name> [--system]");
            Environment.Exit(64);
            return;
        }
        ...
        ServiceResult result = manager.Start(name);
