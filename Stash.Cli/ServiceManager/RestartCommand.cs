...
/// <summary>Implements <c>stash service restart</c>.</summary>
public static class RestartCommand
{
    public static void Execute(string[] args)
    {
        if (args.Length == 0)
        {
            Console.Error.WriteLine("Usage: stash service restart <name> [--system]");
            Environment.Exit(64);
            return;
        }
        ...
        ServiceResult result = manager.Restart(name);
