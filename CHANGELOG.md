...
- **New SA0860 analyzer rule** (Warning, Errors category) — fires when `throw { type: "<built-in name>", ... }` uses a string literal matching a built-in error type name. The struct throw form (`throw ValueError { message: "..." }`) is preferred. Suppress with `// stash-disable SA0860`.
