...
| `bench_text_processing.stash` | Real-world log filtering: substring search, dict-based group-by counting, string slicing/upper/replace |
| `bench_data_transform.stash`  | Real-world CSV-style ETL: split/parse, integer conversion, multi-dict aggregation (count/sum/max/avg), join-serialize |
