...
public static class TomlBuiltIns
{
    ...
    {
        var toml = new StashNamespace("toml");

        // toml.parse(string) — Parses a TOML string into a Stash dict.
        toml.Define("parse", new BuiltInFunction("toml.parse", 1, (_, args) =>
        {
        ...
        toml.Define("stringify", new BuiltInFunction("toml.stringify", 1, (_, args) =>
        {
        ...
        toml.Define("valid", new BuiltInFunction("toml.valid", 1, (_, args) =>
        {
            if (args[0] is not string s)
            {
