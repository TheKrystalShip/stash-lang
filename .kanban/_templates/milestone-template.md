...
- The **Ledger** is *derived, never hand-written*. It is the authority on the *past* (what's
  actually done) and is computed from `4-done/` by `scripts/checkpoint/checkpoint.stash milestone-status`.
...
```bash
stash scripts/checkpoint/checkpoint.stash milestone-status {{milestone-slug}}
```
