...
9. **Hot-path latency.** `DownloadVersion` performs NO synchronous database write on the request thread: it captures the necessary fields, registers a single `Response.OnCompleted` callback, and calls `IDownloadEventQueue.Enqueue` — a non-blocking `Channel.TryWrite`. The actual `download_events` write happens entirely off-request in `MetricsBackgroundService` (which opens its own DI scope via `IServiceScopeFactory` and calls `SaveChangesAsync` there). Verified by two complementary test classes: `DownloadCaptureSemanticsTests` — injects a `FakeDownloadEventQueue` and asserts the request path enqueues exactly the right events (200 → 1 event, 404/hidden → 0, IP routed through `IIpHasher`) with no DB interaction on the request path; and `MetricsBackgroundServiceTests` — directly calls `DrainBatchAsync` on a real `MetricsBackgroundService` and asserts events appear in `download_events` only after the drain call, proving the DB write is decoupled from the HTTP response. No wall-clock or variance assertion is used (the project avoids flaky timing tests).
