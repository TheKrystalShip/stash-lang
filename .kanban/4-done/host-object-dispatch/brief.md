...
| --- | --- | --- |
| Every host-delegate invocation must map CLR exceptions to a `HostError` (the `[StashError]`-registered runtime error, surfaced kind = `StashError.KindHostError`) so they reach the embedder as structured `StashError` rather than raw CLR exceptions. Spans every member kind — Property get, Property set, Method, AsyncMethod. | `InvokeHostDelegate` (private static helper in `Stash.Hosting.Internal`). | **Construct-lite.** `InvokeHostDelegate` is the **only** function in `Stash.Hosting` that calls a registered delegate — all four dispatch paths (Getter, Setter, Method, AsyncMethod) route through it. No API in `Stash.Hosting` reachable today invokes a registered delegate other than through `InvokeHostDelegate`; the wrapping try/catch cannot be bypassed without inventing a parallel invoker. **Structural for sync method dispatch:** `InvokeMethod` is the sole call site for `DynamicInvoke` on a registered sync method handler — the `HostMethodInvoker` closure returns `(object?[] clrArgs, Delegate handler)` so calling the closure directly does not invoke the handler. **Convention-enforced for async method, getter, and setter dispatch:** the property holds because the descriptor fields are dereferenced only inside `InvokeHostDelegate`; the async closure performs its own `DynamicInvoke`+`await` internally (CancellationToken threading and task-bridging cannot be extracted to a simple return value). Same Make-It-Right shape as the MVP's `HostMarshaller` chokepoint: a 5-phase one-project SDK feature does not warrant a manufactured Detect scanner. No meta-test. |
...
| 2026-06-05 | **Exception mapping single source of truth: `InvokeHostDelegate` is the only caller of host delegates.** No meta-test. | Construct-lite — the same Make-It-Right shape as the MVP's `HostMarshaller`. No API in `Stash.Hosting` reachable today invokes a registered delegate other than through `InvokeHostDelegate`; the exception mapping cannot be bypassed without inventing a parallel invoker. Structural for sync method dispatch (`InvokeMethod` is the sole `DynamicInvoke` call site); convention-enforced for async, getter, and setter (the async closure's own `DynamicInvoke`+`await`+CT-bridge cannot be cleanly extracted). Adding a Detect scanner is the manufactured-bandaid pattern this project rejects. |
