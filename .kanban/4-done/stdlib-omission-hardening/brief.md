...
| Concern | Single source of truth | Today's level | Disposition | Verified status (P1 reproduction) | Resolution |
| --- | --- | --- | --- | --- | --- |
| Every `[StashNamespace]` is registered into `GeneratedStdlibRegistry.All()` | `StashNamespaceGenerator.cs:28` (`ForAttributeWithMetadataName`) + `CodeEmitter.cs:334` (`EmitRegistry`) | **Construct** | Keep | Verified — `[StashNamespace]` classes auto-discovered via `ForAttributeWithMetadataName` at `:28`; `EmitRegistry` at `:334` always yields `GeneratedStdlibRegistry.All()` from the collected models; no hand-list. | Keep. Audit cites it as the milestone's reference example. |
| Capability gating per function/member | `CodeEmitter.cs:124` (fn) / `:174` (member) — emitted `if ((__caps & X) == X)` guard | **Construct** | Keep | Verified — emitter unconditionally wraps when a `Capability` is set; tested by `StdlibConsistencyTests.Construction_WithNoCapabilities_*`. | Keep. |
| Duplicate Stash-name dedup within a namespace | `seenFnNames` HashSet in `StashNamespaceGenerator.Build` -> `STASH_GEN005` (`Diagnostics.cs:34`) | **Construct** | Keep | Verified — emits build error. | Keep. |
| `[StashMember]` <-> `[StashFn]` / `[StashConst]` mutual exclusion | `STASH_MEM001` (`Diagnostics.cs:119`) / `STASH_MEM002` (`:130`) build errors | **Construct** | Keep | Verified. | Keep. |
| `<summary>` doc required for `[StashFn]` / `[StashMember]` | `STASH_DOC001` (`Diagnostics.cs:74`, warning) / `STASH_MEM004` (`:153`, error) | **Construct (member) / Detect-ish (fn)** | Keep | Verified — `STASH_DOC001` is `Warning`; `STASH_MEM004` is `Error`. | Keep as-is. The asymmetry is intentional (fn is the older surface); record but do not change in this unit. |
| `<exception>` / `[StashFn(ThrowsTypes=...)]` coverage on throwing builtins | `Stash.Tests/Stdlib/SourceGenerator/Wave1ThrowsCoverageTests` + allow-list + fail-path self-test | **Detect (justified)** | Justify | Verified — file lives under `Stash.Tests/Stdlib/SourceGenerator/` (was cited as directly under `Stdlib/`); listed in `.claude/language-changes.md` enforcement gate. | Keep Detect. Justification: a build-time Roslyn throw-body walk would be heavy and fragile (heuristic over implementation code); the meta-test asserts the load-bearing property and ships a fail-path self-test and an explicit allow-list pinning legitimate infallible reads. Findings file explicitly flags this to push back on. |
| Public completion surface stability | `Stash.Tests/Lsp/CompletionSurfaceSnapshotTests` | **Detect (justified)** | Justify | Verified — file lives under `Stash.Tests/Lsp/` (was cited under `Stdlib/`). | Keep Detect. Justification: the deliberate "conscious re-baseline" property (re-record snapshot via `STASH_SNAPSHOT_REGEN=1`) is what makes the test valuable — replacing it with a computed assertion erases the intentional human review step. |
| Generated reference docs match `StdlibDefinitions` | `Stash.Tests/Stdlib/StandardLibraryReferenceTests` + `Stash.Docs/` regenerator | **Detect (justified)** | Justify | Verified — file confirmed under `Stash.Tests/Stdlib/`. | Keep Detect. Same rationale as snapshots. |
| **GAP A — Silent-skip: `[StashFn]`/`[StashMember]`/`[StashConst]` outside any `[StashNamespace]` class** | `StashNamespaceGenerator.cs:28-29` predicate (filters by `NamespaceAttr` const at `:15` on the *class*; members on a non-namespace class are silently invisible) | **Instruct** today | Promote | Verified — `ForAttributeWithMetadataName(NamespaceAttr)` at `:28` walks only namespace classes; no companion scan for stray fn/member/const attributes elsewhere. | **Promote to Construct.** Add generator diagnostic `STSG014` (confirmed free; existing ids are STSG010/011/013, `STSG012` is a sequence gap). A `[StashFn]` / `[StashMember]` / `[StashConst]` whose declaring type is not `[StashNamespace]`-attributed is a build error. Resolved in phase **2**. |
| **GAP B — Non-exhaustive `Stability` handling** | `Stash.Stdlib.Abstractions/Stability.cs` (enum with two variants today) | **Instruct** today | Promote | Verified — TWO interpretation sites: `Stash.Stdlib/Models/NamespaceMemberPayload.cs:49` (`if (Stability == Stability.Live)` then fall-through Cached path — **line drifted :47 → :49**) and `Stash.Stdlib.Generators/StashNamespaceGenerator.cs:461-464` (`stab == 0 ? Cached : Live`, confirmed exact). A future variant misbehaves silently in both. | **Promote to Construct.** Convert both sites to exhaustive `switch` on `Stability` with a `_ => throw new InvalidOperationException($"unhandled Stability: {stability}")` default. Resolved in phase **3**. |
| **GAP C — DataMembers (`[StashMember]`) lack runtime<->registry consistency tests** | `Stash.Tests/Stdlib/StdlibConsistencyTests.cs` covers Functions / Constants / Namespaces, NOT DataMembers | **Instruct** today | Promote | Verified — exactly twelve `[Fact]`s (GlobalFunctions ×3, Namespaces ×2, NamespaceFunctions ×3, NamespaceConstants ×2, Construction ×2); none enumerate `NamespaceMember` slots in either direction. | **Promote to Detect (with explicit pinning).** Add two new facts: `NamespaceMembers_RegistryEntries_HaveRuntimePayload` and `NamespaceMembers_RuntimePayloads_HaveRegistryMetadata` — same shape as the existing Functions/Constants pairs. (A pure Construct promotion via the generator is impractical here: the runtime side is dispatched through `NamespaceMemberPayload` boxed in a slot, not through a compile-time list the generator can enumerate.) Resolved in phase **4**. |
| **GAP D — UFCS type->namespace map not validated against runtime namespaces** | `Stash.Stdlib/Registry/StdlibRegistry.cs:135` — hard-coded dictionary, **exactly two entries** today (`["string"]="str"`, `["array"]="arr"`) | **Instruct** today | Promote | Verified — initialized in the static ctor at `:135`; nothing asserts the target namespaces exist; the validation target `NamespaceNames` is available at `:28`. | **Promote to Construct.** In the same static ctor, validate every value resolves via `NamespaceNames`. A missing namespace throws a `TypeInitializationException` at first load (fail-closed at process start, before any UFCS dispatch). Resolved in phase **3** (small, sits with the Stability fix). |
| **OBSERVATION — Nullable metadata fields (`BuiltInParam.Type`, `NamespaceMember.ReturnType`, `BuiltInField.Type`)** | `Stash.Stdlib/Models/BuiltInParam.cs:4`, `NamespaceMember.cs:11` (drifted :8 → :11), `BuiltInField.cs:4` (all `string?`) | **Construct already (via emitter)** | No-op | Verified — `CodeEmitter.EmitFunction`/`EmitMember`/`EmitStruct` always emit a non-null type label; no hand-written `NamespaceBuilder` caller exists outside the generator. The `string?` is misleading but harmless. | **No-op.** Flipping to `string` is defense-in-depth, not gap-closure. Recorded in the Decision Log; not implemented. |
| **DEFERRED — Bounded-domain literal scan (stdlib domain)** | Possible: namespace names, type-kind labels, stability labels appearing as inline literals | **Instruct** today | Justify | Partially verified — type-label strings are duplicated across `CodeEmitter.ResolveCSharpTypeForLocal`, `StashNamespaceGenerator.InferStashTypeLabel`, and the runtime marshaller. | **Keep Detect, document — P1 verdict: leave un-promoted.** `Stash.Stdlib.Generators` targets `netstandard2.0` and cannot reference runtime enums, so a shared enum across the boundary is impractical. P1 did **not** exhaustively prove the type-label set is closed and **no concrete desync bug is on record**; per this row's own pre-condition ("add a meta-test only if the set is closed AND the duplication has bitten"), neither holds, so no follow-up meta-test is added in this unit. Re-open if a desync is ever observed. |
| **DEFERRED — "Builtin-shaped method missing `[StashFn]`" heuristic diagnostic** | (none — proposal only) | n/a | Drop | Not pursued. | **Drop.** Findings file already hedges this as fuzzy / likely noise; a noisy build diagnostic is worse than none. Recorded in Decision Log; not implemented. |
...
| 2026-05-31 | **`final_verify` narrowed for documented flakies (during /done).** Added `NamespaceMembersDapTests`, `AtomicClaimRaceTests`, `RegistryAuthzAtomicityConformanceTests` to the exclusion filter. | First `final_verify` run failed 4 tests; an identical re-run failed 3 with a *different* set — an unstable failure set is the flakiness signature. All pass in isolation (Dap 7/7, AtomicClaimRace 2/2). The feature diff touches **zero** Registry/Dap files, so none can be regressions; `NamespaceMembersDapTests` was already excluded in `registry-authz-pdp-completion` (c3ed705). The Registry pair are `ConcurrentFirstPublish` atomicity races (shared-resource contention under parallel xUnit load). |
| 2026-05-31 | **Plan correction (stale verify command).** P1/P2/P3 `verify` used `dotnet build <ProjA> <ProjB>`, which fails with `MSB1008: Only one project can be specified`. Split each into two single-project builds (`Stash.Stdlib.Generators` first, then `Stash.Stdlib`). | `dotnet build` accepts one project/solution; the two-project form never ran. Caught at P1's own verify gate. Whole-solution `default_verify` (`dotnet build`) was unaffected and stayed green. |
| 2026-05-31 | **P1 reproduction complete.** All 15 rows re-verified against the live code; substance holds for every row (4 GAPs real, all Construct/Detect rows reproduce). Four stale citations corrected: `NamespaceMemberPayload.cs` `:47`→`:49`, `NamespaceMember.cs` `:8`→`:11`, `Wave1ThrowsCoverageTests` → `Stash.Tests/Stdlib/SourceGenerator/`, `CompletionSurfaceSnapshotTests` → `Stash.Tests/Lsp/`. Added an explicit **Disposition** column (Keep/Promote/Justify/No-op/Drop). UFCS map confirmed to hold **exactly two** entries (brief's `…` implied more); `STSG014` confirmed unused (`STSG012` is a sequence gap, ids run 010/011/013). DEFERRED bounded-literal row given its P1 verdict: un-promoted (label set not proven closed, no desync on record). | The pre-marked "Verified" claims were themselves treated as unverified hypotheses — the milestone's core discipline. Re-reading each cited `file:line` caught four stale citations and one over-stated cardinality; none changed a disposition, so the resolution plan (P2–P4) is unaffected. |
