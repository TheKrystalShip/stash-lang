...
| 2026-05-23 (P4) | User-module const enforcement does **not** require threading per-export const-ness through `ModuleExports`. SA0845 is applied to any `DotAssignExpr` where the receiver resolves to a `StashNamespace` — built-in or user-module alias. Two existing guarantees make this correct with zero false positives: (1) SA0805 already prohibits exporting mutable `let` bindings, so every legal export is immutable; (2) `ExecuteImportAs` calls `ns.Freeze()` on the alias namespace at module-load time, making the runtime `StashNamespace` receiver always frozen. Treating every `DotAssignExpr` on a namespace receiver as SA0845 is therefore strictly correct. The `ModuleExports.Names` set continues to carry only names (no const bit) — no schema change is needed. | Threading const-ness through `ModuleExports` and the bytecode serializer would require extending the binary format and updating the reader/writer, introducing risk across the serialization boundary with no benefit: the current invariants already ensure correctness. |
| 2026-05-23 (P6) | **LSP completion-item kind for DataMembers: `CompletionItemKind.Property`.** Chosen over `Field` because `Property` matches the C#-property idiom — member access looks like a field read but invokes a getter under the hood. `Field` implies mutable storage; `Constant` is reserved for `[StashConst]` snapshots; `Function` is for callables. Using `Property` clearly separates DataMembers from all three without introducing a custom kind. | The brief listed both Property and Field as defensible. Property is the standard LSP choice for "getter-backed read-only access" (VS Code uses it for C# properties, Python `@property`, etc.) and is unambiguous to IDE users. |
| 2026-05-23 (P6) | **DAP variable-tree labeling convention for DataMembers: `Type = "member"` (Cached) or `Type = "member (live)"` (Live).** Getter is auto-invoked on namespace expansion (confirmed per Open Question 2 — DAP rendering of getter members). Functions retain `Type = "function"`. Constants and enums retain their existing labels. | Auto-invoke on namespace expansion is consistent with showing the current value — the alternative (showing `<member payload>`) would be useless to the user. The `"member"` vs `"member (live)"` split surfaces stability directly in the debugger, matching the `[live]` tag in LSP hover Detail. This makes the labeling round-trip with the hover UI. |
| 2026-05-23 (P6) | **SemanticTokensHandler: DataMember tokens classified as `TokenTypeProperty` with `ModifierDefaultLibrary \| ModifierReadonly`.** This ensures `cli.argc` in source code highlights differently from `cli.parse(` (which gets `TokenTypeFunction`). Functions retain `Function` token type. | The existing walker already classified constants as `Variable + Readonly` and functions as `Function`. Adding a third branch for DataMembers to use `Property + Readonly + DefaultLibrary` is the natural extension — `Property` is the semantic-tokens counterpart to the completion-item-kind choice, making the two surfaces consistent. |
| 2026-05-23 (P6) | **VS Code TextMate grammar and playground Monarch tokenizer: no changes required.** Both tokenizers use dotted-identifier patterns that cover member access identically to function-call identifier positions. The TextMate grammar tags `ns.member` (without `()`) as `variable.other.property.stash` via the existing accessor rule; the Monarch tokenizer falls through to `identifier` for all post-dot identifiers. | Audited `stash.tmLanguage.json` (lines 253–259, 774–799) and `stash-language.js` (lines 102–117). Member access `ns.x` without `()` already uses the same lexical pattern as bare identifiers, so no new rule is needed. Semantic tokens (from `SemanticTokensHandler`) provide the finer distinction at the semantic layer. |
