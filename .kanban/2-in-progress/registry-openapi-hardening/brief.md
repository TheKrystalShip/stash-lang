...
servers:
  # Populated dynamically per request (NOT a static value) — see "Dynamic servers" below.
  - url: "https://<this-deployment-host><BasePath>"
```

**Dynamic `servers` (locked 2026-06-03).** The `servers` array is filled in **at request time** by the document transformer from the incoming request — scheme + host + `BasePath` — honoring `X-Forwarded-Proto`/`X-Forwarded-Host` so a reverse-proxied deployment advertises its public URL, not Kestrel's internal one. The served `/openapi/v1.json` is therefore self-describing on any self-hosted registry with zero operator config (a static `{registryBase}` placeholder was rejected — it would force every operator/consumer to substitute the real URL). Mechanics: `AddHttpContextAccessor()` in `Startup`; the transformer resolves `IHttpContextAccessor` from `OpenApiDocumentTransformerContext.ApplicationServices` and sets `document.Servers` from `HttpContext.Request`. Interaction with the P5 snapshot: the snapshot test must **normalize** `servers[].url` to a fixed placeholder before its byte comparison, since a request-derived URL would otherwise couple the snapshot to the test host/port.
...
**Bounded domains become enums.** Seven `public static class X { public const string Y = "..."; }` declarations in `Stash.Registry.Contracts/BoundedDomains.cs` become `public enum X { Y, ... }` with `[JsonConverter(typeof(JsonStringEnumConverter<X>))]` to activate string serialization **and an explicit `[JsonStringEnumMemberName("y")]` on every member** (the locked mechanism, 2026-06-03 — a lowercase naming policy was rejected: explicit attributes pin the wire string byte-for-byte and survive a future multi-word value that a policy would mangle) so the wire string is byte-identical (`"owner"`, `"publish"`, `"public"`, `"user"`, `"system"`, `"member"`). P4/P5 still **empirically verify the round-trip** holds under both the registry's reflection serializer and the CLI's source-gen + Native-AOT context — what is verified is the round-trip, not the choice of mechanism. DTO property types change from `string` to the new enum types. EF properties pick up `.HasConversion<string>()` value converters and `.HasDefaultValue(MyEnum.Public)`. The CLI's source-gen `CliJsonContext` registers each enum type so AOT serialization round-trips. The `Rank`/`RankOrder`/`IsValid`/`IsReserved` helpers stay (translated to enum-typed signatures).
...
_All four resolved with the user on 2026-06-03 (see Decision Log). What remains below are the implementation-time **verification** tasks the resolutions imply — not open decisions._

- **`/openapi/` JTI bypass — RESOLVED: guard at the top of the existing inline JTI middleware lambda** (`Startup.cs:276`): `if (context.Request.Path.StartsWithSegments("/openapi")) { await next(); return; }`. *Verify in P1* that this leaves all other auth/audit behaviour for non-OpenAPI paths unchanged (integration test). Implication on record: the document is genuinely public — reachable by anyone including revoked-/no-token callers (intended; it exposes API shape, no package data).
- **`servers` — RESOLVED: dynamic per-request** (scheme + host + `BasePath`, honoring forwarded headers); static placeholder rejected. *Verify in P1* that the runtime document transformer can read the request via `IHttpContextAccessor` and that forwarded headers + `BasePath` are honored; *handle in P5* the snapshot host-normalization.
- **OperationId — RESOLVED: convention only** (`{Controller}_{Action}`). No per-action override attribute is built (adding one now is speculative generality; revisit only if a downstream consumer needs name stability across a rename). No verification task.
- **Enum wire-string mechanism — RESOLVED: explicit `[JsonStringEnumMemberName]` per member** (lowercase naming policy rejected). *Verify empirically in P4/P5* that the round-trip holds byte-identical under BOTH the registry's reflection serializer AND the CLI's source-gen context under Native AOT; if they diverge, document the workaround (the choice is locked; only the round-trip is under test).
...
| 2026-06-03 | `servers` is populated **dynamically per request** (scheme + host + `BasePath`, honoring forwarded headers), not a static `{registryBase}` placeholder | The registry is self-hosted — every deployment has a different URL. A request-derived `servers` makes the served doc self-describing with zero operator config; a static placeholder would force every operator/consumer to substitute the real URL. User-resolved (Make-It-Right). Implies `AddHttpContextAccessor()` + a request-aware document transformer (P1) and snapshot host-normalization (P5). |
| 2026-06-03 | OperationId is **convention-only** (`{Controller}_{Action}`); no per-action override mechanism is built | Convention is one predictable rule; a `[ProducesOperationId]`-style override is speculative generality until a real downstream consumer needs name stability across a rename (the doctrine's anti-gold-plating clause). Add it only when that need is concrete. User-resolved. |
| 2026-06-03 | Enum→wire-string mechanism is **explicit `[JsonStringEnumMemberName]` per member** (+ `JsonStringEnumConverter<T>` to activate string mode); lowercase naming policy rejected | Explicit attributes pin each wire string byte-for-byte, are deterministic under both reflection and source-gen + AOT, and survive a future multi-word value a naming policy would mangle. P4/P5 verify the round-trip empirically — the mechanism choice is locked, only the round-trip is under test. User-resolved (Make-It-Right). |
| 2026-06-03 | `/openapi/` JTI bypass placement locked to a path-prefix early-return at the top of the existing inline JTI middleware lambda (`Startup.cs:276`) | The obvious correct placement — touches only `/openapi/*`, leaves every other path's auth/audit flow untouched (verified in P1). Consistent with the public-in-prod decision: the document is reachable by anyone (intended — exposes API shape, no package data). |
