...
| **SpawnedFutureRegistry propagation across child VMs** — every child-VM creation site must thread the registry by reference, otherwise futures spawned by nested async fns / par* callbacks / module loads never register and D1 silently drops their errors | One shared `SpawnedFutureRegistry` field on the **root** VM, threaded into every child VM at construction | **Construct** — the registry is a required ctor / shared-field parameter on the child VM construction path. A child VM without a registry cannot exist (the field is non-nullable; the C# compiler enforces a value at every construction site). Sites enumerated in Design (Async.cs:89, VMContext.Fork, VMContext.cs:667, Modules.cs:119). |
...
| 2026-06-05 | Observation registry placed on the **root** VM; threaded by-reference into every child-VM creation site as a required field | Construct-level prevention of registry-propagation omission; a child VM without a registry cannot exist |
