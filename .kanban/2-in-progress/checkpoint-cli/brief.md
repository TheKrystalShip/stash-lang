...
**Byte-transparent passthrough.** All other invocations shell out via Stash's **non-strict** passthrough command expression `$>(stash ${path} ${args...})`, then `env.exit(result.exitCode)`. `$>` (NOT the strict `$!>`) is required: it streams stdout/stderr live AND returns the child's `CommandResult` with its exit code for *every* code without throwing — so the dispatcher transparently propagates non-zero exits (which the tools, and the side-effect-free parity probes, deliberately produce). A strict `$!>` would throw `CommandError` on the first non-zero child instead, breaking passthrough. (Empirically confirmed in phase 1A; see the 2026-06-02 Decision Log entry.) stdout and stderr stream live (no capture-and-reprint, which can mangle trailing newlines and re-order interleaved streams). Argument vector is passed through verbatim — including `--selftest`, exotic flags, and free-form strings for verify commands.
...
| 2026-06-02 | **No PATH shim (user decision); migration target is the runnable `stash scripts/checkpoint/checkpoint.stash <sub>` form.** | The Summary's "docs read `checkpoint <sub>`" and the Non-Goal's "no PATH plumbing" (line 43) are mutually exclusive — a bare `checkpoint` is only runnable with something named `checkpoint` on PATH. The Open Question (line 180) deferred the `bin/checkpoint` shim; the user chose **not** to add it (keep verbose). Therefore **every migration in phases 3A–4C rewrites `stash scripts/checkpoint/<sub>.stash` to `stash scripts/checkpoint/checkpoint.stash <sub>`** (runnable from repo root, guard-clean because `checkpoint.stash` is internal). Wherever this brief or `plan.yaml`'s `done_when` writes the shorthand `checkpoint <sub>` (e.g. 3A's "`checkpoint validate-spec`"), it denotes that runnable form — never a bare `checkpoint` command, which does not exist. A hint string or doc command that is not literally runnable is a defect. |
| 2026-06-02 | **`RAWPATH_SCAN_EXCLUSIONS` permanently holds TWO files: `_registry.stash` and `lint_common_imports.stash`.** | The plan's "at minimum `_registry.stash`" anticipated documented additions. `lint_common_imports.stash` (the guard's own source) legitimately contains raw subcommand-path literals: its header `//` usage examples reference `scripts/checkpoint/lint_common_imports.stash` (the `lint` subcommand's own path), and during 3A–4C its `RAWPATH_EXEMPTIONS` data held subcommand-file paths. Scanning the guard against its own pattern would self-flag, so it is permanently excluded alongside the registry data file. This is the "explicit Decision Log entry" 5A's criterion 2 requires for the second exclusion; the list is unchanged since 2A. (`RAWPATH_EXEMPTIONS`, the temporary migration list, is a separate const and is empty as of 4C.) |
