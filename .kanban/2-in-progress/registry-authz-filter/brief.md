...
- **No status/body/PDP-outcome changes.** Every status code, `ErrorResponse` body, and PDP allow/deny outcome is identical before and after this feature. `RegistryAuthzMatrixTests` is the gate. **One sanctioned, additive exception — audit coverage is unified** (user decision, 2026-05-30): the single filter audits *every* authenticated denial, whereas today the Orgs/Scopes controllers do not audit denials (Auth/Admin/Packages do). A single filter cannot preserve a per-controller disagreement; it must pick. This *adds* audit rows for previously-unaudited authenticated denials (e.g. `CreateOrg`) and never removes one. Auditing authorization denials is a security positive and the prior omission was an oversight, not policy. The added coverage is gated by new `RegistryAuthzAuditMutationTests` rows (P2). See the Decision Log entry "uniform deny-audit".
...
- **Audit unification covered.** `RegistryAuthzAuditMutationTests` gains rows asserting that an authenticated denial on each previously-unaudited clean endpoint converted by this feature (at minimum `CreateOrg`, one Orgs member/team endpoint, and `VerifyScope`) now writes exactly one audit entry. This locks in the sanctioned uniform-audit behavior, which `RegistryAuthzMatrixTests` (status/body only) cannot see. The test runs in P2's verify and in `final_verify`.
...
| 2026-05-30 | **Uniform deny-audit (additive).** The shared filter audits every authenticated denial, including Orgs/Scopes endpoints that did not audit denials before. Supersedes the literal "every audit row identical" wording in Non-Goals. | The six controllers disagreed — Auth/Admin/Packages audited authenticated denials; Orgs/Scopes did not. One filter cannot preserve a disagreement; it must pick. Auditing authz denials is a security positive and the omission read as an oversight, not policy. Additive only (never removes a row). New `RegistryAuthzAuditMutationTests` rows gate the added coverage; surfaced when P1's `CreateOrg` pilot began auditing. **User decision.** |
| 2026-05-30 | **Authorization-before-validation ordering (accepted).** The `RegistryAuthorizeFilter` runs as an MVC authorization filter, which fires before model binding and the action body. As a result, a caller who fails BOTH authorization AND body validation now receives the authorization failure (401/403) before the body-validation failure (400). This is the conventional ASP.NET MVC filter-pipeline order (authorization filters run before model binding / the action body), and it is more secure — untrusted bodies are not parsed for callers who will be rejected regardless. The PDP allow path and all status/body outcomes for well-formed requests are unchanged. The ordering shift applies to every PDP-then-validate endpoint converted in P2: `CreateOrg`, `AddOrgMember`, `RemoveOrgMember`, `CreateTeam`, `AddTeamMember`, `AssignRole` (`AssignPackageRole`), `RevokeRole` (`RevokePackageRole`), `SetVisibility` (`ChangePackageVisibility`), and others. The `RegistryAuthzMatrixTests` and `RegistryAuthzAuditMutationTests` suites are unaffected because they do not exercise the malformed-body + insufficient-ceiling cell. **User decision.** |
