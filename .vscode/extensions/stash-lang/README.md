...
✦ **LSP** — 27 handlers · **DAP** — 18 handlers · **Test Explorer** — TAP v14 · **Snippets** — 46 templates · **Semantic Highlighting** — 12 token types
...
Built on OmniSharp, the Stash LSP provides 27 handlers covering the full editing lifecycle.
...
- Parses `test.describe`, `test.it`, and `test.skip` blocks to build a hierarchical test tree
...
- **Run Suite** — run a single `test.describe` block and its children
- **Run Test** — run a single `test.it` case
...
46 code snippets for common patterns (34 for Stash, 12 for templates). Trigger via `Ctrl+Space` or by typing the prefix.
...
| `test.it`         | Test case                      |
| `test.describe`   | Describe block (group tests)   |
| `test.skip`       | Skip a test case               |
| `test.beforeAll`  | Before-all lifecycle hook      |
| `test.afterAll`   | After-all lifecycle hook       |
| `test.beforeEach` | Before-each lifecycle hook     |
| `test.afterEach`  | After-each lifecycle hook      |
| `aeq`             | Assert equal                   |
| `atrue`           | Assert true                    |
...
| `stash.lsp.debounceTime`          | `25`              | Debounce delay (ms) before running analysis after a document change.                                                             |
| `stash.lsp.logLevel`              | `"warning"`       | Minimum log level for the language server (`trace`, `debug`, `information`, `warning`, `error`).                                 |
| `stash.trace.server`              | `"off"`           | Trace level for LSP communication (`off`, `messages`, `verbose`).                                                                |
| `stash.inlayHints.enabled`        | `true`            | Enable or disable inlay hints (parameter name hints at call sites).                                                              |
| `stash.codeLens.enabled`          | `true`            | Enable or disable code lens (reference counts above functions, structs, and enums).                                              |
