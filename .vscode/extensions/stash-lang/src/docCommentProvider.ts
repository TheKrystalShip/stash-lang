...
const FUNCTION_DECLARATION_RE =
  /^(?:export\s+)?(?:async\s+)?fn\s+[a-zA-Z_]\w*\s*\(/;
...
export function registerDocCommentExpansionProvider(): vscode.Disposable {
...
export function buildDocCommentSnippetForContext(
  lineText: string,
...
export function parseFunctionSignature(line: string): FunctionSignature | null {
...
export function buildDocCommentSnippet(
  indent: string,
...
function isFunctionDeclarationPrefix(prefix: string): boolean {
...
function isDocCommentLine(lineText: string): boolean {
...
function splitTopLevel(text: string, separator: string): string[] {
  ...
  let quote: string | null = null;
  let escaped = false;
  const depth: NestingDepth = { paren: 0, bracket: 0, brace: 0 };
...
function updateNestingDepth(depth: NestingDepth, char: string): void {
  if (char === OPEN_PAREN) {
    depth.paren++;
  } else if (char === CLOSE_PAREN) {
