...
public static class UnobservedFaultReporter
{
    ...
    /// block to <paramref name="errorOutput"/>. No-op when no unobserved faults exist.
    /// </summary>
    /// <param name="registry">The registry to scan.</param>
    /// <param name="errorOutput">Where to write the warning (typically stderr).</param>
    /// <returns>The number of unobserved faults reported (0 if none).</returns>
    public static int Report(SpawnedFutureRegistry registry, TextWriter errorOutput)
    {
