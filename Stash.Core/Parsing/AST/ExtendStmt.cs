...
/// <c>dict</c>, <c>int</c>, <c>float</c>) and user-defined structs. The target may also be a
/// namespace-qualified type (<c>extend diff.DiffOptions { ... }</c>). Extension methods receive an
...
public class ExtendStmt : Stmt
{
    ...
    /// <summary>Gets the type expression identifying the type being extended.</summary>
    public TypeExpression TypeName { get; }
    ...
    /// <param name="typeName">The type expression identifying the type being extended.</param>
    ...
    public ExtendStmt(Token extendKeyword, TypeExpression typeName, List<FnDeclStmt> methods, SourceSpan span) : base(span, StmtType.Extend)
    {
