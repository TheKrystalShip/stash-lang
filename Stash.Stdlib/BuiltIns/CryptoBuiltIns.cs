...
public static partial class CryptoBuiltIns
{
    ...
    /// <exception cref="ValueError">if `algo` is not one of: md5, sha1, sha256, sha512</exception>
    ...
    /// <exception cref="ValueError">if `algo` is not one of: md5, sha1, sha256, sha512</exception>
    /// <exception cref="TypeError">if `algo` is not a string</exception>
    /// <returns>The hash as a lowercase hexadecimal string</returns>
    [StashFn]
    ...
    /// <exception cref="ValueError">if `n` is not greater than 0, exceeds the maximum size, or `encoding` is not one of: hex, base64, raw</exception>
    /// <exception cref="TypeError">if `encoding` is not a string</exception>
    ...
    /// <exception cref="ValueError">if `bits` is not 128, 192, or 256</exception>
    /// <exception cref="TypeError">if `bits` is not an integer</exception>
    ...
    /// <exception cref="ValueError">if the key is not 32 bytes (256-bit)</exception>
    /// <exception cref="ParseError">if the key is provided as a hex string with an invalid format</exception>
    /// <exception cref="TypeError">if `data` is not a string or byte[], or `key` is not a hex string or byte[]</exception>
    ...
    /// <param name="options">Reserved for future use (optional)</param>
    /// <exception cref="ValueError">if the key is not 32 bytes, the combined hex string is too short, or authentication tag verification fails</exception>
    /// <exception cref="ParseError">if any hex field in the ciphertext dict or the key contains invalid hex characters</exception>
    /// <exception cref="TypeError">if `ciphertext` is not a dict or hex string, required dict fields are missing or not strings, or `key` is not a hex string or byte[]</exception>
