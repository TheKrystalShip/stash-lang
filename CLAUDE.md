...
- **Roslyn meta-tests must be load-order-deterministic.** Build the `CSharpCompilation`'s `MetadataReference`s from `TRUSTED_PLATFORM_ASSEMBLIES` (+ the specific assemblies under test), **never** `AppDomain.CurrentDomain.GetAssemblies()` — its set varies with test execution order, so the scan can bind nothing and pass *vacuously* (0 violations is the success condition). Pair the file-count floor (`MinScannedFiles`) with a **binding-floor**: assert a known target type resolves to a non-error symbol, so "0 findings because nothing bound" fails loudly instead of masquerading as "0 findings because clean." (`CliNoMagicWireStringsMetaTests` shipped this bug and was fixed to this shape.)
...
**`dotnet build` takes exactly ONE project/solution arg.** A phase `verify:` / `default_verify:` line like `dotnet build Stash.Hosting Stash.Core` fails `MSB1008` ("Only one project or solution"); use a single project or `dotnet build Stash.sln` (a green superset). Recurring `plan.yaml`-authoring trap — corrected in-flight on host-object-dispatch (P2/P5), swept across 4 commands in registry-api-readiness.

**Background `run-verify` bare — never `| tail` it.** A shell pipeline's exit code is `tail`'s (≈always 0), so a fail-closed `FAIL` is reported as success, *and* `tail` truncates the `Failed: N` verdict line. Run it under the Bash tool's `run_in_background` with **no pipe** (the harness captures full output to the task file), then `grep`/read that file for the verdict. (A `| tail -40` once masked a 40-test regression as exit-0 here.) The same applies to `promote-done` and `worktree-finish`, which run `final_verify` internally — background them bare on a cold tree for the same reason.
...
**Avoid 3-deep agent nesting on `.stash` phases — skip the implementer.** A phase whose work is pure `.stash` authoring, dispatched orchestrator→`implementer`→`stash-author`, can stream-idle-timeout: the implementer's stream sits idle for the *entire* nested `stash-author` call (one ~minutes-long gap). For such phases, go **orchestrator→`stash-author` directly** (one level) and let the orchestrator run the phase mechanics itself — `verify-phase`, the feat commit, `advance-checkpoint`, the chore commit. (The orchestrator is bound by the same docs-first protocol whether or not an implementer is in the chain.) Mechanical doc migrations need no agent at all: a deterministic `sed` sweep inline is faster and more reliable than an LLM re-reading many occurrences.

### Running features in parallel

Default: **one feature on `main` at a time.** Never run two agents against the same working tree — the clean-tree invariant means feature A's in-progress edits block feature B's workflow commands, and same-file edits silently overwrite with no git conflict to flag the loss. When two features must progress concurrently, give each its own git worktree + `feature/<slug>` branch, run the whole lifecycle (including `/spec`) on the branch, and integrate with `git merge --no-ff`. Parallelize only across **disjoint subsystems** (e.g. language + registry); serialize features that share hot files (two language features both touch `TokenType.cs`, `Parser.cs`, and every visitor). Re-run `final_verify` on `main` after every merge — green-on-branch does not imply green-on-merged-`main`. Full procedure: `.claude/WORKFLOW.md` → "Running Features in Parallel".
